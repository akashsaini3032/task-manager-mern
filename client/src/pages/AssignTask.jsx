...
const AssignTask = () => {
    ...
    const ans = mydata.map((key) => {
        no++;
        return (
            <tr key={key._id} className="assigntask-row">
                <td>{no}</td>
                <td>{key.name}</td>
                <td>{key.email}</td>
                <td>{key.designation}</td>
                <td>
                    <Button variant="outline-info" className="assigntask-btn" onClick={() => { handleShow(key._id) }}>
                        Assign Task
                    </Button>
                </td>
            </tr>
    ...
    return (
        <div className="assigntask-wrapper">
            <h2 className="assigntask-title">Assign Task To User</h2>
            <hr className="assigntask-divider" />

            <div className="assigntask-table-wrapper">
                <Table bordered hover responsive className="assigntask-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>User Name</th>
            ...
            <Modal show={show} onHide={handleClose} centered className="assigntask-modal">
                <Modal.Header closeButton>
                    <Modal.Title>Assign Task</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form className="assigntask-form" onSubmit={handleSubmit}>
                        ...
                        <Button variant="primary" type="submit" className="assigntask-submit-btn">
                            Submit
                        </Button>
                    </Form>
                </Modal.Body>
